(function () {
  ...
  function loadScript(src) {
    if (window.ecoLoadScript) {
      return window.ecoLoadScript(src);
    }
  ...
  function ensureReact() {
    ...
    return loadScript(REACT_URL).then(function () {
      return loadScript(REACT_DOM_URL);
    });
  }

  function bootTransportWidget() {
    var React = window.React;
    var ReactDOM = window.ReactDOM;
    var root = document.getElementById('transport-times-widget');
    if (!root || root.dataset.mounted === '1' || !React || !ReactDOM) {
      return;
    }
    ...
    var TransportButton = memo(function TransportButton(props) {
      var item = props.item;
      var data = props.data;
      var Icon = item.Icon;
      var tooltip = formatBadgeText(item, data);
    ...
    function TransportTimesWidget() {
      var timesState = React.useState(null);
      var times = timesState[0];
      var setTimes = timesState[1];
  ...
  function init() {
    ...
    if (window.ecoWhenVisible) {
      window.ecoWhenVisible(root, scheduleBoot, { rootMargin: '400px' });
      return;
    }
